...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5]">
      <header className="sticky top-0 z-30 bg-[#3A187F] px-4 py-3 sm:px-6">
        <div className="mx-auto max-w-[1450px]">
          <label className="relative mx-auto block w-full max-w-2xl">
            ...
            <input
              className="h-10 w-full rounded-[10px] border-0 bg-white py-2 pl-11 pr-4 text-sm text-slate-800 outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-white/50"
              type="search"
      ...
      <main className="mx-auto max-w-[1450px] px-4 py-6 sm:px-6 lg:py-8">
            <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                className="inline-flex h-10 items-center gap-2 self-start rounded-[10px] bg-[#3A187F] px-4 text-sm font-semibold text-white transition-colors hover:bg-[#2f1269] focus:outline-none focus:ring-2 focus:ring-[#3A187F]/25 disabled:cursor-not-allowed disabled:opacity-60"
                type="button"
            ...
            {message && (
              <p className="mb-4 rounded-[10px] border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{message}</p>
            )}
            ...
            <section className="min-w-0 overflow-hidden rounded-[10px] border border-slate-200 bg-white">
              <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="rounded-full bg-[#3A187F]/8 px-2 py-0.5 text-xs font-semibold text-[#3A187F]">{filteredProducts.length}</span>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    className={`h-8 rounded-[10px] border px-3 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-[#3A187F]/15 ${
                      sort.field === 'precio'
                        ? 'border-[#3A187F] bg-[#3A187F] text-white'
                        : 'border-slate-200 bg-white text-slate-600 hover:border-[#3A187F]/30 hover:text-[#3A187F]'
                    }`}
                    type="button"
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {loading ? (
                      ...
                      filteredProducts.map((product) => (
                        <tr key={product.id} className="transition-colors hover:bg-[#3A187F]/[0.025]">
                          ...
                          <td className="px-5 py-3.5">
                            <div className="flex justify-end gap-1">
                              <button
                                className="rounded-[10px] px-2.5 py-1.5 text-xs font-semibold text-[#3A187F] transition-colors hover:bg-[#3A187F]/8 focus:outline-none focus:ring-2 focus:ring-[#3A187F]/15"
                                type="button"
            ...
            <section className="min-w-0 overflow-hidden rounded-[10px] border border-slate-200 bg-white">
              <div className="border-b border-slate-200 px-5 py-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="rounded-full bg-[#3A187F]/8 px-2 py-0.5 text-xs font-semibold text-[#3A187F]">{alertas.length}</span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/40 p-4 backdrop-blur-[2px]" role="presentation" onMouseDown={closeModal}>
          ...
          >
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              {modal === 'add' && (
                <>
                  <label className="block text-sm font-medium text-slate-700">
                    Nombre
                    <input className="mt-1.5 h-10 w-full rounded-[10px] border border-slate-300 bg-white px-3 text-sm outline-none transition focus:border-[#3A187F] focus:ring-2 focus:ring-[#3A187F]/10" name="nombre" value={form.nombre} onChange={handleChange} maxLength="100" required />
                  </label>
                  <label className="block text-sm font-medium text-slate-700">
                    Precio
                    <input className="mt-1.5 h-10 w-full rounded-[10px] border border-slate-300 bg-white px-3 text-sm outline-none transition focus:border-[#3A187F] focus:ring-2 focus:ring-[#3A187F]/10" name="precio" type="number" min="0" step="0.01" value={form.precio} onChange={handleChange} required />
                  </label>
                  <label className="block text-sm font-medium text-slate-700">
                    Descripción
                    <textarea className="mt-1.5 w-full resize-y rounded-[10px] border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none transition focus:border-[#3A187F] focus:ring-2 focus:ring-[#3A187F]/10" name="descripcion" value={form.descripcion} onChange={handleChange} rows="3" />
                  </label>
                </>
              )}
              ...
              <label className="block text-sm font-medium text-slate-700">
                Cantidad
                <input className="mt-1.5 h-10 w-full rounded-[10px] border border-slate-300 bg-white px-3 text-sm outline-none transition focus:border-[#3A187F] focus:ring-2 focus:ring-[#3A187F]/10" name="cantidad" type="number" min="0" step="1" value={form.cantidad} onChange={handleChange} required />
              </label>
              ...
              <div className="flex justify-end gap-2 border-t border-slate-100 pt-5">
                ...
                <button className="h-10 rounded-[10px] bg-[#3A187F] px-4 text-sm font-semibold text-white transition hover:bg-[#2f1269] disabled:cursor-not-allowed disabled:opacity-60" type="submit" disabled={saving}>
