...
const categoryMeta = {
  'Cliente Alto Nivel': { shortLabel: 'Alto nivel', color: '#7a2746', badge: 'bg-[#f4e2e8] text-[#6f203e] dark:bg-rose-500/15 dark:text-rose-200' },
  'Cliente Normal': { shortLabel: 'Normal', color: '#c992a6', badge: 'bg-slate-100 text-slate-600 dark:bg-white/10 dark:text-slate-300' },
  'Cliente En Riesgo': { shortLabel: 'En riesgo', color: '#e7c6d1', badge: 'bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-200' },
}
...
function PersonIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
      ...
      <path d="M4.8 20c.5-4 3-6 7.2-6s6.7 2 7.2 6" strokeLinecap="round" />
    </svg>
  )
}
...
function RankCard({ eyebrow, client, variant }) {
  ...
  return (
    <article className={`relative overflow-hidden rounded-2xl border p-5 shadow-sm ${
      isRisk
        ? 'border-amber-100 bg-gradient-to-br from-white to-amber-50/70 shadow-amber-950/5 dark:border-amber-400/15 dark:from-[#242424] dark:to-amber-950/20'
        : 'border-rose-100 bg-gradient-to-br from-white to-[#faeef2] shadow-rose-950/5 dark:border-rose-400/15 dark:from-[#242424] dark:to-rose-950/20'
    }`}>
      <div className={`absolute -right-7 -top-7 size-28 rounded-full ${isRisk ? 'bg-amber-100/60 dark:bg-amber-500/10' : 'bg-rose-100/70 dark:bg-rose-500/10'}`} />
      <div className="relative">
        <div className="mb-5 flex items-center justify-between gap-3">
          ...
          <span className={`grid size-9 place-items-center rounded-xl ${isRisk ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-200' : 'bg-[#f1dce3] text-[#7a2746] dark:bg-rose-500/15 dark:text-rose-200'}`}>
            <span className="size-5"><PersonIcon /></span>
          </span>
        </div>
...
function CustomersPage() {
  ...
  const donutBackground = useMemo(() => {
    if (!distribution.total) return '#eee3e7'
    let accumulated = 0
    ...
    <main className="mx-auto min-h-full max-w-[1450px] px-4 py-6 sm:px-6 lg:py-8">
      <header className="mb-6 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button className="h-10 rounded-xl border border-rose-200 bg-white px-4 text-sm font-semibold text-[#7a2746] transition hover:bg-rose-50 disabled:opacity-60 dark:border-rose-400/20 dark:bg-[#242424] dark:text-rose-200 dark:hover:bg-rose-500/10" type="button" onClick={loadCustomers} disabled={loading}>
            {loading ? 'Actualizando…' : 'Actualizar'}
          </button>
          <button className="h-10 rounded-xl bg-[#7a2746] px-4 text-sm font-semibold text-white shadow-sm shadow-rose-950/20 transition hover:bg-[#671d3a]" type="button" onClick={() => { setError(''); setMessage(''); setShowModal(true) }}>
            Registrar pedido
          </button>
        </div>
      </header>
      ...
      {!backendReady && (
        <div className="mb-5 flex items-start gap-3 rounded-xl border border-rose-200 bg-white/80 px-4 py-3 text-sm text-slate-600 shadow-sm dark:border-rose-400/15 dark:bg-[#242424] dark:text-slate-300">
      ...
      {message && <p className="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-400/20 dark:bg-emerald-500/10 dark:text-emerald-200">{message}</p>}
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-3">
        {[
          ...
          <article className="rounded-2xl border border-rose-100 bg-white/85 p-5 shadow-sm shadow-rose-950/5 dark:border-rose-400/15 dark:bg-[#242424]" key={label}>
      ...
      <section className="mb-6 grid gap-6 lg:grid-cols-[minmax(330px,0.8fr)_minmax(0,1.2fr)]">
        <article className="rounded-2xl border border-rose-100 bg-white p-5 shadow-sm shadow-rose-950/5 dark:border-rose-400/15 dark:bg-[#242424] sm:p-6">
          ...
          <div className="mt-7 flex flex-col items-center gap-7 sm:flex-row sm:justify-center">
            <div className="relative grid size-44 shrink-0 place-items-center rounded-full" style={{ background: donutBackground }}>
              <div className="grid size-28 place-items-center rounded-full bg-white text-center shadow-inner dark:bg-[#242424]">
      ...
      <section className="overflow-hidden rounded-2xl border border-rose-100 bg-white shadow-sm shadow-rose-950/5 dark:border-rose-400/15 dark:bg-[#242424]">
        <div className="flex flex-col gap-4 border-b border-rose-100 px-5 py-4 dark:border-rose-400/15 lg:flex-row lg:items-end lg:justify-between">
          <div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <label className="relative">
              ...
              <input className="h-10 w-full rounded-xl border border-rose-100 bg-[#fcf8f9] pl-9 pr-3 text-sm outline-none transition placeholder:text-slate-400 focus:border-[#b75c7e] focus:ring-2 focus:ring-rose-100 dark:border-rose-400/15 dark:bg-white/5 dark:focus:ring-rose-900 sm:w-64" type="search" placeholder="Buscar por nombre o correo" value={search} onChange={(event) => setSearch(event.target.value)} />
            </label>
            <select className="h-10 rounded-xl border border-rose-100 bg-[#fcf8f9] px-3 text-sm text-slate-600 outline-none focus:border-[#b75c7e] focus:ring-2 focus:ring-rose-100 dark:border-rose-400/15 dark:bg-[#242424] dark:text-slate-300 dark:focus:ring-rose-900" value={status} onChange={(event) => setStatus(event.target.value)}>
              <option>Todos</option>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] border-collapse text-left text-sm">
            <thead className="bg-[#fbf5f7] dark:bg-[#202020]">
              <tr className="border-b border-rose-100 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:border-rose-400/15 dark:text-slate-500">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4 backdrop-blur-[2px]" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setShowModal(false) }}>
          <div className="w-full max-w-md rounded-2xl border border-rose-100 bg-white p-6 shadow-2xl dark:border-rose-400/15 dark:bg-[#242424]" role="dialog" aria-modal="true" aria-labelledby="order-title">
            <div className="mb-5">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300">ID del cliente
                <input className="mt-1.5 h-11 w-full rounded-xl border border-rose-100 bg-[#fcf8f9] px-3 outline-none focus:border-[#b75c7e] focus:ring-2 focus:ring-rose-100 dark:border-rose-400/15 dark:bg-white/5 dark:focus:ring-rose-900" type="number" min="1" required value={form.id_usuario} onChange={(event) => setForm((current) => ({ ...current, id_usuario: event.target.value }))} />
              </label>
              <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300">Monto total
                <div className="relative mt-1.5"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">$</span><input className="h-11 w-full rounded-xl border border-rose-100 bg-[#fcf8f9] pl-7 pr-3 outline-none focus:border-[#b75c7e] focus:ring-2 focus:ring-rose-100 dark:border-rose-400/15 dark:bg-white/5 dark:focus:ring-rose-900" type="number" min="0.01" step="0.01" required value={form.monto_total} onChange={(event) => setForm((current) => ({ ...current, monto_total: event.target.value }))} /></div>
              </label>
              <div className="flex justify-end gap-2 border-t border-rose-100 pt-5 dark:border-rose-400/15">
                ...
                <button className="h-10 rounded-xl bg-[#7a2746] px-4 text-sm font-semibold text-white hover:bg-[#671d3a] disabled:opacity-60" type="submit" disabled={saving}>{saving ? 'Guardando…' : 'Guardar pedido'}</button>
              </div>
            </form>
          </div>
        </div>
